...
try {
    ...
    Write-Host "  ⚠ GitHub CLI not found (optional - will use manual method)" -ForegroundColor Yellow
}

if (-not $prereqsOk) {
    Write-Host "`nPlease fix the issues above and run again." -ForegroundColor Red
    exit 1
}

# Step 2: Get GitHub credentials if not provided
Write-Host "`n[2/6] Setting up GitHub..." -ForegroundColor Yellow
...
if ($ghCliAvailable) {
    ...
    try {
        gh repo create $RepositoryName --public --source=. --remote=origin --push 2>&1 | Out-Null
        if ($LASTEXITCODE -eq 0) {
            Write-Host "  ✓ Repository created and code pushed!" -ForegroundColor Green
            $repoCreated = $true
        }
    } catch {
        Write-Host "  ⚠ Could not create repo via CLI, will set up manually" -ForegroundColor Yellow
    }
}

# Step 4: Set up Git remote (if not created by GitHub CLI)
if (-not $repoCreated) {
    Write-Host "`n[3/6] Setting up Git remote..." -ForegroundColor Yellow
    ...
    if ($remoteExists) {
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Host "  ⚠ Could not add remote. You may need to create the repository first." -ForegroundColor Yellow
...
Write-Host "`n[4/6] Pushing to GitHub..." -ForegroundColor Yellow
...
if ($remoteExists) {
    try {
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            $pushOutputStr = $pushOutput | Out-String
            if ($pushOutputStr -match "authentication") {
                Write-Host "  ⚠ Authentication required. Options:" -ForegroundColor Yellow
                ...
                Write-Host "  ⚠ Push failed. You may need to create the repository first." -ForegroundColor Yellow
                Write-Host "  Create it at: https://github.com/new?name=$RepositoryName" -ForegroundColor Cyan
            }
        }
    } catch {
        Write-Host "  ⚠ Push failed. See error above." -ForegroundColor Yellow
    }
} else {
    Write-Host "  ⚠ No remote configured. Please create repository first." -ForegroundColor Yellow
}

# Step 6: Set up self-hosted runner
Write-Host "`n[5/6] Setting up self-hosted runner..." -ForegroundColor Yellow

$remoteExists = git remote -v 2>&1 | Select-String "origin"
if ($remoteExists) {
    $repoUrl = (git remote get-url origin) -replace '\.git$', ''
    Write-Host "  Repository URL: $repoUrl" -ForegroundColor Cyan
    Write-Host "`n  To set up the runner:" -ForegroundColor Yellow
    ...
    Write-Host "  ⚠ Set up runner after repository is created and pushed" -ForegroundColor Yellow
}

# Step 7: Initial local deployment
Write-Host "`n[6/6] Setting up local environment..." -ForegroundColor Yellow
...
try {
    ...
    Write-Host "`n  Deploying locally..." -ForegroundColor Yellow
    docker-compose down 2>&1 | Out-Null
    docker-compose build 2>&1 | Out-Null
    
    Write-Host "  ✓ Docker image built" -ForegroundColor Green
    Write-Host "`n  To start the service, run:" -ForegroundColor Cyan
    Write-Host "    docker-compose up -d" -ForegroundColor White
    Write-Host "  Or use: .\scripts\deploy-local.ps1" -ForegroundColor White
} catch {
    Write-Host "  ⚠ Docker Compose not found. Install Docker Desktop." -ForegroundColor Yellow
}
...
if ($remoteExists) {
    $repoUrl = (git remote get-url origin) -replace '\.git$', ''
    Write-Host "`n✓ Repository: $repoUrl" -ForegroundColor Green
    ...
    Write-Host "`n⚠ Repository not set up yet" -ForegroundColor Yellow
    Write-Host "`nNext steps:" -ForegroundColor Yellow
